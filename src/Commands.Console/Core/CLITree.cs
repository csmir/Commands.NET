...
public static class CLITree
{
    ...
    /// </summary>
    /// <param name="tree">The <see cref="IExecutionProvider"/> instance that should be used to run the CLI command.</param>
    ...
    public static void Run<T>(this IExecutionProvider tree, CLIOptions<T> options)
        where T : ConsoleCallerContext, new()
    {
        options.Caller ??= new T();

        tree.TryExecute(options.Caller, ArgumentArray.Read(options.Arguments), options.Options);
    }
    ...
    public static void Run<T>(this IExecutionProvider tree, T caller, string[] args)
        where T : ConsoleCallerContext, new()
    {
