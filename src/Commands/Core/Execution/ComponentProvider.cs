...
public class ComponentProvider : IComponentProvider
{
    ...
    /// </summary>
    /// <remarks>
    ///     If no handlers are provided, a default handler is added which allows the command result to be processed with no further implications. In order not to use the default handler, provide <see cref="ResultHandler.Empty"/> as the only argument.
    /// </remarks>
    ...
    /// <inheritdoc />
    public virtual Task Execute<TContext>(TContext context, CommandOptions? options = null)
    ...
    /// <summary>
    ///     A protected method that fires the command pipeline using the provided context and options, and handles the returned results according to the defined <see cref="Handlers"/>.
    /// </summary>
    /// <remarks>
    ...
    protected virtual async Task Work<TContext>(TContext context, CommandOptions options)
        where TContext : class, ICallerContext
    {
        options.Provider ??= this;

        var result = await Run(context, options).ConfigureAwait(false);
    ...
    /// <summary>
    ///     A protected method that executes the command pipeline using the provided context and options. It will attempt to find available commands and execute them in order until any succeed. If none succeed, an erroneous result is returned.
    /// </summary>
    /// <remarks>
    ///      This method is called by the <see cref="Work{TContext}"/> method after setting the source of the execution, and can be overridden to provide custom behavior.
    /// </remarks>
    /// <typeparam name="TContext">The implementation type of <see cref="ICallerContext"/> which represents the caller (or source) of the execution request.</typeparam>
    /// <param name="context">The implementation of <see cref="ICallerContext"/> which represents the caller (or source) of the execution request.</param>
    /// <param name="options">The options used to customize the command execution pipeline in accordance to the context and requirements of execution.</param>
    /// <returns>An awaitable <see cref="ValueTask{TResult}"/> with an <see cref="IResult"/> which contains the result of the operation.</returns>
    protected virtual async ValueTask<IResult> Run<TContext>(TContext context, CommandOptions options)
        where TContext : class, ICallerContext
    {
        IResult? result = null;
