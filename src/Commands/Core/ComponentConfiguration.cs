...
namespace Commands
{
    ...
    public sealed class ComponentConfiguration
    {
        ...
        public ComponentConfiguration(IEnumerable<TypeConverter> converters, string? namingPattern = @"^[a-z0-9_-]*$", Dictionary<string, object>? properties = null)
            : this(converters.ToDictionary(x => x.Type), properties ?? [], namingPattern is not null ? new Regex(namingPattern) : null) { }

        internal ComponentConfiguration(Dictionary<Type, TypeConverter> converters, Dictionary<string, object> properties, Regex? namingPattern)
        {
