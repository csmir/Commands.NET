...
namespace Commands
{
    ...
    public sealed class ComponentConfiguration
    {
        ...
        public ComponentConfiguration(IEnumerable<TypeConverter> converters, string? namingPattern = @"^[a-z0-9_-]*$", bool sealModuleDefinitions = false)
            : this(converters.ToDictionary(x => x.Type), [], namingPattern is not null ? new Regex(namingPattern) : null, sealModuleDefinitions) { }

        internal ComponentConfiguration(Dictionary<Type, TypeConverter> converters, Dictionary<string, object> properties, Regex? namingPattern, bool enforceReadonly, Func<IComponent, bool>? filter = null)
        {
